...
namespace lantern{
    ...
    namespace backprop {
         ...
         * @param _regularization Regularization function define in lantern::regularization namespace
         * @param _lambda Lambda for L1 and L2 regularization
         * @param _batch_size Batch size
         */
        template <typename Optimizer, typename RegularizationFunc>
        void Backpropagate(
            ...
            RegularizationFunc& _regularization,
            ...
            for(uint32_t current_layer = (*all_layer_sizes).size() - 1; current_layer > 0; current_layer--){
                ...
                gradient_weight += _regularization(
                    _lambda,
                    batch_size,
                    weights
                );
        ...
        void Backpropagate(
            ...
            for(uint32_t current_layer = (*all_layer_sizes).size() - 1; current_layer > 0; current_layer--){
                ...
                all_gradient /= static_cast<double>(batch_size);
                uint32_t opt_index = current_layer - 1;
                af::array delta = _optimizer.GetDelta(all_gradient,opt_index);
                parameters -= delta;
                parameters.eval();
        ...
        void Backpropagate(
            ...
            for(uint32_t current_layer = (*all_layer_sizes).size() - 1; current_layer > 0; current_layer--){
                ...
                all_gradient /= static_cast<double>(_batch_size);
                uint32_t opt_index = current_layer - 1;
                af::array delta = _optimizer.GetDelta(all_gradient,opt_index);
                parameters -= delta;
                parameters.eval();
